...
class _LikedPageState extends State<LikedPage> {
  ...
  Future<void> _loadLiked() async {
    ...
    try {
      final data = await _api.getLiked(email: email);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(_error ?? "Failed to load liked")),
      );
    }
  }

  Future<void> _removeFromLiked(String restaurantId) async {
    final email = Session.email.trim();
    if (email.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Removed from liked ✅")),
      );
